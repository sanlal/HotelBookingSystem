...
-- Triggers
DELIMITER $$
CREATE TRIGGER after_booking_update_status
...
BEGIN
	IF ( 
        ...
        OR
        OLD.check_out_date IS NULL AND NEW.check_out_date IS NOT NULL
	) THEN
		UPDATE room SET is_avail = TRUE WHERE room_no = NEW.room_no;
	END IF;
END$$
DELIMITER ;
